...
/**
 * Get all carts started by a particular user
 *
 * @param    {Object}     reqData     contains keys for params, body, query, headers, method
 ...
 * @return   {void}
 */
CartsAPI.prototype.findByUser = function (reqData, successCb, errorCb) {
  /** 
   * Handles the cart data
   *
   * @param  {Array}  data  The carts data array or null if query conditions weren't matched
   */
  var successHandler = function handleSuccess(data) {
    if(data === null) {
      data = []
    }
    
    this.intercom.emit('facet:response:cart:data', data);
  }.bind(this);

  if(_.isFunction(successCb)) { successHandler = successCb; }
  var conditions = { user_id: reqData.params.userId };
  conditions = this.scopeConditions(conditions);
  this.find({conditions: conditions}, successHandler, errorCb);
